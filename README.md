...
> **生产环境注意**
>
> - `APP_ENV=production` 时不会再写入演示账号种子数据。
> - 生产环境禁止使用上述默认演示账号密码登录；请在上线前修改或删除演示账号。
> - 若生产库中仍保留默认密码，API 启动时会输出告警日志。
> - **全新生产部署**请通过环境变量创建首个管理员（仅在库中还没有任何 admin 时生效）：
>
> ```env
...
> ```
>
> 创建成功后可从环境变量中移除密码；后续管理员请在后台用户管理中邀请或改角色。
